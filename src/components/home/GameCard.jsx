    ...
    >
      ...
      {isFinished && rank > 0 && (
        <div style={{ position: 'absolute', top: 10, left: 14, zIndex: 1, display: 'flex', alignItems: 'center', gap: 3 }}>
          <span style={{
            ...
            color: rank === 1 ? '#FFD700' : rank === 2 ? '#C0C0C0' : rank === 3 ? '#CD7F32' : 'var(--db-text-ghost)',
          }}>{ordinal(rank)}</span>
          {rank <= 3 && (
      ...
      <div style={{ position: 'absolute', top: 10, right: 10, zIndex: 1, display: 'flex', alignItems: 'center', gap: 4 }}>
        {isLive ? (
          ...
          <span style={{ fontFamily: 'var(--db-font-display)', fontSize: 10, letterSpacing: '0.1em', color: 'var(--db-text-muted)', background: 'var(--db-bg-elevated)', border: '1px solid var(--db-border-default)', borderRadius: 4, padding: '2px 7px' }}>
            FINAL
          </span>
        ) : null}
      </div>
      ...
      >
        <div>
          {isLive ? (
            ...
            <>
              <span style={{ fontFamily: 'var(--db-font-display)', fontSize: 11, letterSpacing: '0.06em', color: 'var(--db-text-muted)' }}>FINAL</span>
              {game.away_score != null && game.home_score != null && (
                <div style={{ fontFamily: 'var(--db-font-mono)', fontSize: 13, fontWeight: 700, color: 'var(--db-text-ghost)', marginTop: 2, fontVariantNumeric: 'tabular-nums' }}>
            ...
            <>
              {formatDateLabel(game.starts_at) && (
                <span style={{ fontFamily: 'var(--db-font-ui)', fontSize: 10, fontWeight: 500, letterSpacing: '0.04em', color: 'var(--db-text-ghost)', textTransform: 'uppercase', display: 'block' }}>
                  {formatDateLabel(game.starts_at)}
                </span>
              )}
              <span style={{ fontFamily: 'var(--db-font-mono)', fontSize: 13, fontWeight: 600, color: 'var(--db-text-secondary)' }}>
